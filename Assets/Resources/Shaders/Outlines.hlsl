...
scharr_operators get_edge_detection_kernels()
{
...
void depth_based_outlines_float(float2 screen_uv, float2 px, out float outlines)
{
	...
	scharr_operators kernals = get_edge_detection_kernels();
	float gx = 0;
	float gy = 0;
	for (int i = -1; i <= 1; i++) {
		for (int j = -1; j <= 1; j++) {
			...
			float d = SampleSceneDepth(screen_uv + offset);
...
void normal_based_outlines_float(float2 screen_uv, float2 px, out float outlines)
{
	...
	scharr_operators kernels = get_edge_detection_kernels();
	float gx = 0;
	float gy = 0;
	float3 cn = SampleSceneNormals(screen_uv);
	for (int i = -1; i <= 1; i++){
		for (int j = -1; j <= 1; j++){
			if (i ==0 && j == 0) continue;
			float2 offset = float2(i, j) * px;
			float3 n = SampleSceneNormals(screen_uv + offset);
			float dp = dot(cn, n);
